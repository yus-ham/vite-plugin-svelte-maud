...
import { read_script } from "../read.js";
import { closing_tag_omitted, decode_character_references } from "svelte/compiler/parse-utils/html.js";
import { is_void } from "svelte/compiler/parse-utils/names.js";
...
/** @type {Map<string, import('svelte/compiler').SvelteNode['type']>} */
...
/** @param {import('svelte/compiler').TemplateNode[]} stack */
...
function parent_is_shadowroot_template(stack) {
	...
	while (i--) {
		if (
			...
			/** @type {import('svelte/compiler').RegularElement} */ (stack[i]).attributes.some(
...
export default function element(parser) {
	...
	if (root_only_meta_tags.has(name)) {
		if (is_closing_tag) {
			if (
				...
				/** @type {import('svelte/compiler').ElementLike} */ (parent).fragment.nodes.length
			) {
				e.svelte_meta_invalid_content(
					/** @type {import('svelte/compiler').ElementLike} */ (parent).fragment.nodes[0].start,
					name
				);
			}
		} else {
	...
	const element =
		type === 'RegularElement'
			? {
					type: /** @type {import('svelte/compiler').ElementLike['type']} */ (type),
					start,
					end: -1,
					name,
					attributes: [],
	...
	if (is_closing_tag) {
		...
		while (/** @type {import('svelte/compiler').RegularElement} */ (parent).name !== name) {
	...
	if (is_void(name)) {
		...
		try {
			...
			e.message = `Void element (${name}) must be terminated by semicolon`;
			throw e
		}
	...
	if (element.type === 'SvelteComponent') {
		...
		const definition = /** @type {import('svelte/compiler').Attribute} */ (
	...
	if (is_top_level_script_or_style) {
		...
		/** @type {import('svelte/compiler').Comment | null} */
		...
		let content
		if (name === 'script') {
			content = read_script(parser, start, element.attributes);
			if (prev_comment) {
			...
			content = read_style(parser, start, element.attributes);
    ...
    if (name === 'textarea') {
		...
		try { parser.eat('{', true) }
			...
			e.message = `Expected open curly brace "{" after tag name but found "${parser.template[parser.index]}"`;
			throw e;
		}
...
function create_fragment(transparent) {
    return {
        type: 'Fragment',
        nodes: [],
        transparent,
...
const regex_whitespace_or_semicolon = /[\s;]/;
...
function read_tag_name(parser) {
	...
	const name = parser.read_until(regex_whitespace_or_semicolon);
...
function read_static_attribute(parser) {
	...
	/** @type {true | Array<import('svelte/compiler').Text | import('svelte/compiler').ExpressionTag>} */
	let value = true;
...
/**
 * @returns {import('svelte/compiler').Attribute | import('svelte/compiler').SpreadAttribute | import('svelte/compiler').Directive | null}
 */
function read_attribute(parser) {
	...
	if (parser.eat('(')) {
		...
		if (parser.eat('...')) {
			...
			/** @type {import('svelte/compiler').SpreadAttribute} */
			const spread = {
			...
			/** @type {import('svelte/compiler').ExpressionTag} */
			const expression = {
				type: 'ExpressionTag',
				start: value_start,
	...
	if (type) {
		...
		/** @type {import('svelte/compiler').Directive} */
		// @ts-expect-error TODO can't figure out this error
		const directive = {
			start,
			end,
...
function read_sequence(parser, done, location) {
	...
	/** @type {Array<import('svelte/compiler').Text | import('svelte/compiler').ExpressionTag>} */
	const chunks = [];
